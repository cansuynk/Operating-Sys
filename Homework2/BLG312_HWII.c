...
void mySignal(int sigNum){ printf("\nReceived signal with num = %d.\n", sigNum); }
...
void detach_memory(int *mem_ptr){
    if(shmdt((void *)mem_ptr) < 0){
        printf("shmdt error...\n");
        printf("errno: %d\n", errno);
        exit(EXIT_FAILURE);
    }
    printf("\nmemory is detached successfully...\n");
}
...
int main(int argc, char *argv[]){
 ...
 printf("Hi");
    ...
    const key_t KEYSHM = ftok(argv[0], 2);          // Creation of key value for shared memory.
